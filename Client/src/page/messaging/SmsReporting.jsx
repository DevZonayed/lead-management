...
import SendMessageDrawer from "../../components/DataGrid/subComponents/DrawerComponents/SendMessageDrawer";
...
const SmsReporting = () => {
          ...
          if (res.data.lead) {
            if (res.data.lead.length === 0) {
              ...
              let contacts = res.data.lead.map((item) => {
                return { name: item?.name, phone: item?.phone[0] };
              });
  ...
  useEffect(() => {
    (async function () {
      ...
      let response = await getMessageReport("sms");
      setReports({
  ...
  const handleAddToContactFromSearchResult = (item) => {
    ...
    if (itemIndex !== -1) {
      toast.dismiss();
      toast.error(item.phone + " is already exist in sort list!");
      return;
    }
    setContactData((prev) => {
      return [
  ...
  const handleMessageDrawer = () => {
    ...
    drawer.show(<SendMessageDrawer data={sendAbleData} />);
  };
  ...
  return (
    <div>
      <div className="h4">Sms Department</div>
      <div className="row">
        <div className="col-12 col-lg-6">
          <div className="card">
            <div className="card-header">
              <div className="h5">Send Message</div>
            </div>
                  ...
                  {searchContacts.length === 0 ? (
                    ...
                    <>
                      ...
                      {searchContacts.map((item, index) => {
                        return (
                          <div
                            key={"contactTable" + index + item.phone}
          ...
          <div className="card">
            <div className="card-header">
              <div className="header">
                ...
                >
                  <div className="h5">
                    SMS Reports {reports.isLoading ? "| Loading..." : ""}
                  </div>
