...
const SendMailDrawer = ({ data }) => {
  ...
  const options = useMemo(() => {
    let optionList = [];
    lead.lead.map((item) => optionList.push(...item.email));
    return optionList;
  }, [lead]);
  const { getAllEmailTemplate } = useAxios();
  // States
    ...
    if (msgType === " text") {
      if (text.length <= 15) {
        if (
          ...
          throwMailToAll(mails, mailSub, text, "text");
          return;
        } else {
          return false;
        }
      ...
      throwMailToAll(mails, mailSub, html, "html");
      return;
    }
  };
   ...
   */
  function throwMailToAll(mails, mailSub, message, type) {
