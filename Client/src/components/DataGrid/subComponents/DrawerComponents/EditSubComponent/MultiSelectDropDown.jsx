...
const MultiSelectDropDown = ({
  ...
  const id = useId();
  const { subjects, settings } = useSelector((state) => state);
  const [defaultValues, setValues] = useState();
  // const [pillEditor, setPillEditor] = useState(null);
  const [ref, pillEditor, setPillEditor] = useComponentVisible(false);
  ...
  useEffect(() => {
    ...
    const generateOptions =
      settings.data[type] !== undefined ? settings.data[type] : [];

    setOptions(isArray ? SelectOptions : generateOptions);
  }, [defaultValue, subjects, SelectOptions, isArray, settings, type]);
      ...
      <div className="interest_pill_wrapper d-flex">
        {defaultValues !== undefined &&
          ...
          (defaultValues.length <= 15
            ...
            if (!isArray) {
              return (
                <div ref={ref} key={index + "interest_item"}>
